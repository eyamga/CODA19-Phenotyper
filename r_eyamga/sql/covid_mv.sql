...
WHERE
	datetime (intervention_start_time) BETWEEN datetime (covidepisodes.episode_start_time, '-1 day') AND datetime (covidepisodes.episode_end_time) -- we only consider MV event that occured during the COVID admission