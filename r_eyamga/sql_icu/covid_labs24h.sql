...
FROM
	icu_episodes
	INNER JOIN lab_data ON icu_episodes.patient_site_uid = lab_data.patient_site_uid
...
WHERE
	datetime (lab_sample_time) BETWEEN datetime (icu_episodes.episode_start_time, '-24 hour') 
	AND datetime (icu_episodes.episode_start_time, '+24 hour')
)
