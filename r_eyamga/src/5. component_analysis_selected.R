...
coda19 <- coda19%>%select(c("patient_age", "hemoglobin_min", "plt_max", "wbc_mean", "neutrophil_mean", 
...
write_csv(as_tibble(pca.coda19$eig, rownames='PCA comp')%>%head(10), "./output/pca/pca_selected_eigenvalues.csv")
...
write_csv(as_tibble(pca.coda19$var$contri, rownames='Variables')%>%head(10), "./output/pca/pca_selected_correlationscsv")
...
pca.scree
dev.print(pdf, file="./output/pca/pca_selected_screeplot.pdf");
...
cos2plot
dev.print(pdf, file="./output/pca/pca_selected_cos2plot.pdf")
...
pca_varplot
dev.print(pdf, file="./output/pca/pca_selected_pcavarplot.pdf")
...
pca_indplot_bydeath
dev.print(pdf, file="./output/pca/pca_selected_pcaindplot_death.pdf")
...
pca_indplot_byicu
dev.print(pdf, file="./output/pca/pca_selected_pcaindplot_icu.pdf")
...
pca_indplot_bymv
dev.print(pdf, file="./output/pca/pca_selected_pcaindplot_mv.pdf")
...
pca_indplot_byloc
dev.print(pdf, file="./output/pca/pca_selected_indplot_byloc.pdf")
...
pca_indplot_bywave
dev.print(pdf, file="./output/pca/pca_selected_indplot_bywave.pdf")
...
mca.coda19 <- MCA(coda19_categorical,
                  quali.sup = c(22:24), # Removing death, icu and wave from observation but MV, LOC kept
                  graph = FALSE)

# Tabular info only
# 1) Eigen values
write_csv(as_tibble(mca.coda19$eig, rownames='MCA comp')%>%head(10), "./output/mca/mca_selected_eigenvalues.csv")

print(mca.coda19$eig)

# 2) MCA all contributions
write_csv(as_tibble(mca.coda19$var$contri, rownames='Variables')%>%head(100)%>%arrange(desc(.[[2]], .[[3]], .[[4]], .[[5]])), "./output/mca/mca_selected_contributions.csv")
...
mca_cos2plot <- ggcorrplot::ggcorrplot(t(mca.coda19$var$cos2),
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  labs(title = "Cos 2 of variables",x = "Principal Components", y = "Variables",  xfill = "Cos2 values") +
  scale_fill_gradient2(breaks=c(0, 0.8), limit = c(0,0.8), low = "white", mid ="#89efc7",  high =  "#1e6076", midpoint = 0.2)

mca_cos2plot
dev.print(pdf, file="./output/mca/mca_selected_cos2plot.pdf")



# 3) mca Variables plot
mca_varplot <- fviz_mca_var(mca.coda19,
                            col.var = "contrib", # Color by contributions to the PC alternatives, color by cos2
                            invisible = "quali.sup",
                            gradient.cols = c(low ="#89efc7",  high =  "#1e6076"),
                            ggtheme = theme_minimal(),
  ...
  labs(title = "MCA Variables Plot", x = "PC1", y = "PC2", color = "Contribution %")

mca_varplot
dev.print(pdf, file="./output/mca/mca_selected_mcavarplot.pdf")


# Clustering of the variables
# Create 3 groups of variables (centers = 3)
# set.seed(123)
# coda19.kmeans.onvar <- kmeans(mca.coda19$var$coord, centers = 3, nstart = 25)
# coda19.var.clusters <- as.factor(coda19.kmeans.onvar$cluster)
# 
# # Color variables by groups
# fviz_mca_var(mca.coda19, col.var = coda19.var.clusters, 
#              palette = c("#0073C2FF", "#EFC000FF", "#868686FF"),
#              legend.title = "Cluster")


# Observations and mca

# 4) mca Individual plot

# Labeling observation using outcome variable

# Observations classified
# By death
mca_indplot_bydeath <- fviz_mca_ind(mca.coda19,
                                    geom.ind = "point", # show points only (nbut not "text")
                                    col.ind = coda19_continuous$death, # color by death
                                    palette = c("#00AFBB", "#E7B800", "#FC4E07"),
                                    addEllipses = TRUE, # Concentration ellipses
  ...
  labs(title = "MCA Individuals Plot", x = "PC1", y = "PC2")

mca_indplot_bydeath
dev.print(pdf, file="./output/mca/mca_selected_mcaindplot_death.pdf")




#By icu (NB does not represent current ICU admission but all ICU admissions)
mca_indplot_byicu <- fviz_mca_ind(mca.coda19,
                                  geom.ind = "point", # show points only (nbut not "text")
                                  col.ind = coda19_continuous$icu, # color by death
                                  palette = c("#00AFBB", "#E7B800", "#FC4E07"),
                                  addEllipses = TRUE, # Concentration ellipses
...
mca_indplot_byicu
dev.print(pdf, file="./output/mca/mca_selected_mcaindplot_icu.pdf")

#By MV
mca_indplot_bymv <- fviz_mca_ind(mca.coda19,
                                 geom.ind = "point", # show points only (nbut not "text")
                                 col.ind = coda19_continuous$mv, # color by death
                                 palette = c("#00AFBB", "#E7B800", "#FC4E07"),
                                 addEllipses = TRUE, # Concentration ellipses
...
mca_indplot_bymv
dev.print(pdf, file="./output/mca/mca_selected_mcaindplot_mv.pdf")


#By Level Of Care

mca_indplot_byloc <- fviz_mca_ind(mca.coda19,
                                  geom.ind = "point", # show points only (nbut not "text")
                                  col.ind = coda19_continuous$levelofcare, # color by death
                                  #habillage = c(53), alternative script valid for factoextrar mca objects
                                  palette = c("#00AFBB", "#E7B800", "#FC4E07"),
                                  addEllipses = TRUE, # Concentration ellipses
                                  legend.title = "Level Of Care"
) + labs(title = "MCA Individuals Plot", x = "PC1", y = "PC2")
...
mca_indplot_byloc
dev.print(pdf, file="./output/mca/mca_selected_indplot_byloc.pdf")


#By Wave

mca_indplot_bywave <- fviz_mca_ind(mca.coda19,
                                   geom.ind = "point", # show points only (nbut not "text")
                                   col.ind = coda19_continuous$wave, # color by death
                                   #habillage = c(53), alternative script valid for factoextrar mca objects
                                   palette = c("#00AFBB", "#E7B800", "#FC4E07"),
...
mca_indplot_bywave
dev.print(pdf, file="./output/mca/mca_selected_indplot_bywave.pdf")
...
famd.coda19 <- FAMD(coda19,
                    sup.var = c(40:42), # Removing death, icu and wave from observation but MV, LOC kept
                    graph = FALSE)


# Tabular info only
# 1) Eigen values
write_csv(as_tibble(famd.coda19$eig, rownames='FAMD comp')%>%head(10), "./output/famd/famd_selected_eigenvalues.csv")

# 2) FAMD all contributions
write_csv(as_tibble(famd.coda19$var$contri, rownames='Variables')%>%head(100)%>%arrange(desc(.[[2]], .[[3]], .[[4]], .[[5]])), "./output/famd/famd_selected_contributions.csv")
...
# Adding categorical for variables
...
famdindbydeath <- fviz_famd_ind(famd.coda19,
              axes = c(1,2),
             geom = "point",
             geom.ind = "point",
             habillage = "death",
...
famdindbydeath
dev.print(pdf, file="./output/famd/famd_selected_famdindplot_death.pdf")



# FAMD by icu
famdindbyicu <- fviz_famd_ind(famd.coda19,
                                axes = c(1,2),
                                geom = "point",
                                geom.ind = "point",
                                habillage = "icu",
...
famdindbyicu
dev.print(pdf, file="./output/famd/famd_selected_famdindplot_icu.pdf")
